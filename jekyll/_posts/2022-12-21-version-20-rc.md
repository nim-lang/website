...
- The experimental `nimPreviewDotLikeOps` switch is going to be removed or deprecated because it was bad designed.
...
is used so that the `E1` or `E2` prefixes can be left out in many cases. Those features are most beneficial for independently developed libraries.
