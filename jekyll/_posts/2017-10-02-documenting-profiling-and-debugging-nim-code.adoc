...
may not be immediately evident by looking at the API of libraries or even
the software's source code.

There are many ways to document code. You will know from the previous chapter
that,
...
Multi-line comments are delimited by `discard """` and `"""`.
...
// TODO: Different multi-line comment syntax will be supported in Nim soon.
...
_Listing 5.11_ shows a very simple documentation comment. The Nim compiler
includes a command to generate documentation for a given module. Save the code
in _Listing 5.11_ as `test.nim` somewhere on your file system then
...
it find all the documentation comments and then goes through each of them.
...
_Table 5.1_ shows some example syntax of the reStructuredText markup language.
...
// TODO: Fix this table.
// TODO: Wtf? Works with two backticks.
// TODO: Broken in asciidoctor-pdf :( "Cells with source code listing" http://asciidoctor.org/docs/user-manual/#cell
...
.The resulting documentation for _Listing 5.12_
image::ch05_math_docs.png[]

As you can see from the example in _Listing 5.12_, documentation comments
...
them underneath as shown in _Figure 5.11_.

This is how the Nim standard library is documented. For more examples on how
to document your code you can look at its source.
...
most of its time, or which procedures call which other procedures.
...
In the report shown in _Listing 5.14_, the profiler has made 195 snapshots.
...
in the `ab` procedure, this makes sense because the string passed to the
...
profiler results show the `diff` procedure.

It is easy to determine where the bulk of the processing takes place in
_Listing 5.13_ without the use of a profiler. But for more complex modules
...
In many cases profilers are not cross-platform. If you are a Windows user
then I'm afraid you will not be able to use Valgrind.
...
Let's try Valgrind on the example application in _Listing 5.13_. Recompile the
application without any flags: `nim c main` then execute the following
command to run it under Valgrind: `valgrind --tool=callgrind -v ./main`.
...
The textual output given by the callgrind tool is very large and so it is not
practical looking at it all in a text editor. Thankfully a tool exists to
...
to get something similar to the screenshot in _Figure 5.12_.

.QCacheGrind showing the call graph of _Listing 5.13_
image::ch05_qcachegrind.png[]

The results of the Callgrind tool show many more calls during the lifetime of
the example application in _Listing 5.13_. This is because many of the C
functions which have been defined by Nim when translating the Nim code to C
are now visible. These functions are necessary to implement the behaviour of
the code in _Listing 5.13_.
...
The output from Callgrind gives you much more low-level details about the
execution of your Nim applications. _Figure 5.12_ shows the number of times
...
greater complexity so Valgrind is a much more difficult to use tool than the
Nim profiler.

== Debugging Nim code

Debugging is one of the most important activities in software development.
Bugs in software occur inadvertantly. A user reports an issue that they
are having with your software. So how do you fix this issue?
...
Type in some text and observer the results. You should see something similar
to the output in _Listing 5.16_ below.
...
exception.
<3> The exception message followed by the exception type in square brackets.
...
the program exiting. The stack trace in _Listing 5.16_ is very informative,
it leads directly to the line which caused the crash. Beside the `adder.nim`
module name, the number `3` is a line number pointing to that line number
in the `adder` module. This line is highlighted in _Listing 5.17_ below.
...
when a stack trace does not give enough information about the issue, there
may be multiple variables interacting with each other to produce the crash.

The primary purpose of debugging is to investigate the value of the data
...
By far the simplest approach is to use the `echo` procedure. The `echo`
...
data. Running the example in _Listing 5.18_ and typing in 3 Tab
...
The exception message just shows some whitespace which is what Tab characters
are shown as in normal text. But you have no way of distinguishing whether
that whitespace is just normal space characters or whether it is in fact a
number of Tab characters. The `repr` procedure solves this ambiguity by showing
...
Compiling the code in _Listing 5.20_ will fail with an error:
...
all you need to do use replace `echo` with `debugEcho` and the code will
compile.
****
...
called LLDB. LLDB is a much newer debugger than GDB, but it functions in almost
exactly the same way as GDB.

Let's try to use GDB (or LLDB if you're on Mac OS X) to debug the small
`adder.nim` example introduced in _Listing 5.15_ above. I will repeat the
example below.
...
and you should see something similar to _Figure 5.13_ or _Figure 5.14_.
...
The debugger will then stop the execution of the program at line 3. Figures
_5.15_ and _5.16_ show what that will look like.
...
value of the `line` variable. Due to the fact that the `line` variable
has been defined in line 2. Displaying the value of a variable is the same in
both GDB and LLDB.
...
You should see something similar to the screenshot in _Figure 5.17_.
...
that you found from the previous step. Figures _5.18_ and _5.19_ show what
you may see.
...
can display. Figures _5.20_ and _5.21_ show what this looks like in GDB
...
This is much more complicated than simply using the `echo` procedure. But you
should now have an idea of how to compile your Nim program so that it can
be debugged using GDB and LLDB. These debuggers are useful in certain
circumstances, and there are many more features that
these debuggers provide which are beyond the scope of this book. These
...
This brings you to the end of another chapter. This chapter mainly focused on
the Nimble package manager including example usage of the `nimble` command
line application, the process involved in creating Nimble packages and how
these packages can be published to Nimble's package repository. I hope that
now you have a good idea of the purpose of Nimble and the ways in which it
helps you develop Nim applications. This chapter also looked at other tools
in the Nim ecosystem and it also explained how to use them.

This chapter covered:

  * The installation of the Nimble package manager.
  * Instructions on how to use the Nimble command-line application.
  * Creating, installing, publishing and developing Nimble packages.
  * Documenting code using reStructuredText.
  * Profiling code using the Nim profiler and Valgrind.
  * Debugging Nim code using `echo`, stack traces, GDB and LLDB.

In the next chapter you will find out how concurrency works in Nim. By looking
at a number of examples and seeing how they can be made to utilize concurrency
you will learn about concurrency features in Nim and in which circumstances
those features should be used.
